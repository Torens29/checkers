    ...
    if(table.rows[focusEl.row +deltRow ].cells[focusEl.cell +deltCell] && orederTo){
        ...
        // проверка на пустую ячейку
            ...
            //проверка шашки на битость
                ...
                //проверка на границу поля и на пустату x2 клетки 
                    if(!table.rows[focusEl.row + Math.sign(deltRow)*(Math.abs(deltRow) +1)].cells[focusEl.cell +Math.sign(deltCell)*(Math.abs(deltCell)+1)].childNodes.length){
                        //проверка на не свою шашку за битой
                        table.rows[focusEl.row +Math.sign(deltRow)*(Math.abs(deltRow)+1 )]
                        .cells[focusEl.cell +Math.sign(deltCell)*(Math.abs(deltCell)+1)].classList.add("move_option_beat");
...
function beatOfChip(cellClick){
    table.rows[focusEl.row + (focusEl.distantOfBeatChip* Math.sign(cellClick.parentNode.rowIndex - focusEl.row))]
        ...
        table.rows[focusEl.row + (focusEl.distantOfBeatChip* Math.sign(cellClick.parentNode.rowIndex - focusEl.row))]
        .cells[focusEl.cell + (focusEl.distantOfBeatChip * Math.sign(cellClick.cellIndex - focusEl.cell))].style.background = 'green' ;
